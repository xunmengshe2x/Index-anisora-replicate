...
def attention_forward_default(self, hidden_states, mask, **kw_args):
    ...
    if "text_length_split" in kw_args and "sp_gp" in kw_args and kw_args["sp_gp"] != None:
        ...
        query_layer = _remove_extra_encoder(q, _sp_text_length, _sp_gp.size())
        key_layer = _remove_extra_encoder(k, _sp_text_length, _sp_gp.size())
        value_layer = _remove_extra_encoder(v, _sp_text_length, _sp_gp.size())

    context_layer = attention_fn(query_layer, key_layer, value_layer, mask, dropout_fn, **kw_args)

    if _sp_text_length != None and _sp_gp != None:
        _hidden_states = _add_extra_encoder(context_layer, _sp_text_length, _sp_gp.size())
