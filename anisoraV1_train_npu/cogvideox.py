...
class Basic3DPositionEmbeddingMixin(BaseMixin):
    ...
    def reinit(self, parent_model=None):
        ...
        pos_embed = get_3d_sincos_pos_embed(self.pos_embedding.shape[-1], self.height, self.width,
    ...
    def attention_fn(
        ...
        if self.pnp:
            ...
            query_layer = torch.cat(
                (query_layer[:, :, :kwargs['text_length'], ], self.rotary(query_layer[:, :, kwargs['text_length']:, ])),
                dim=2)
            key_layer = torch.cat(
                (key_layer[:, :, :kwargs['text_length'], ], self.rotary(key_layer[:, :, kwargs['text_length']:, ])),
                dim=2)
            if self.rot_v:
                value_layer = torch.cat((value_layer[:, :, :kwargs['text_length'], ],
                                         self.rotary(value_layer[:, :, kwargs['text_length']:, ])), dim=2)
...
class AdaLNMixin(BaseMixin):
    def __init__(
        ...
        if qk_ln:
            self.query_layernorm_list = nn.ModuleList(
                [LayerNorm(hidden_size_head, eps=1e-6, elementwise_affine=elementwise_affine) for _ in
                 range(num_layers)])
    ...
    def attention_fn(self, query_layer, key_layer, value_layer, attention_mask,
                     ...
                     old_impl=attention_fn_default, **kwargs):
        ...
        return old_impl(query_layer, key_layer, value_layer, attention_mask, attention_dropout=attention_dropout,
                        log_attention_weights=log_attention_weights, scaling_attention_score=scaling_attention_score,
                        **kwargs)
...
class DiffusionTransformer(BaseModel):
    def __init__(
        ...
        if use_SwiGLU:
            self.add_mixin('swiglu', SwiGLUMixin(num_layers, hidden_size, self.inner_hidden_size, bias=False),
                           reinit=True)
    ...
    def _build_modules(self, module_configs):
        ...
        self.add_mixin('pos_embed', instantiate_from_config(
            pos_embed_config, height=self.latent_height // self.patch_size, width=self.latent_width // self.patch_size,
        ...
        self.add_mixin('patch_embed', instantiate_from_config(
            patch_embed_config, patch_size=self.patch_size,
            hidden_size=self.hidden_size,
            in_channels=self.in_channels), reinit=True)
        if self.input_time == 'adaln':
            adaln_layer_config = module_configs['adaln_layer_config']
            self.add_mixin('adaln_layer',
                           instantiate_from_config(
                               adaln_layer_config, height=self.latent_height // self.patch_size,
                               width=self.latent_width // self.patch_size,
                               hidden_size=self.hidden_size, num_layers=self.num_layers,
                               compressed_num_frames=(
                                                             self.num_frames - 1) // self.time_compressed_rate + 1,
        ...
        self.add_mixin('final_layer', instantiate_from_config(
            final_layer_config, hidden_size=self.hidden_size,
