...
def add_model_config_args(parser):
    ...
    # training data mode
    group.add_argument('--training-from-vae-encoded-data', type=bool, required=False, default=False, 
                help='If training on vae encoded train data.')

    # mock vae encoded data
    group.add_argument('--mock-vae-encoded-data', type=bool, required=False, default=False, 
                help='mock vae encoded data for traning.')
    group.add_argument('--mock-vae-encoded-config', type=dict, required=False, default=None)
...
def initialize_distributed(args):
    ...
    mpu.initialize_model_parallel(args.model_parallel_size, timeout=_exce_timeout)

    from sgm.util import set_context_parallel_group, initialize_context_parallel
    from sat.helpers import print_rank0

    if None != args.model_config:
        try:
            _cp_size = None
            _cp_size = args.model_config["first_stage_config"]["params"]["cp_size"]
        except Exception as e:
            print_rank0(repr(e))
        finally:
            _cp_size = _cp_size if None == _cp_size else max(1, _cp_size)

    print_rank0(f"initialize_context_parallel with {_cp_size}")
    if _cp_size != None:
        set_context_parallel_group(args.model_parallel_size, mpu.get_model_parallel_group())
        # if 1 != args.model_parallel_size:
        #     # Set vae context parallel group equal to model parallel group
        #     set_context_parallel_group(args.model_parallel_size, mpu.get_model_parallel_group())
        # else:
        #     initialize_context_parallel(_cp_size, timeout=_exce_timeout)

    print_rank0(f"initialize_sp_parallel with {args.sp_size}")
    if args.sp_size > 0:
        from sgm.dist_group_mgr import DistGpMgr
        from transformer_sp.util import LABEL_D_TRANSFORMER
        DistGpMgr.add(LABEL_D_TRANSFORMER, args.sp_size, _exce_timeout)
    else:
        assert False, "Invalid: args.sp_size."
    ...
    if args.deepspeed:
        ...
        deepspeed.init_distributed(
            dist_backend=args.distributed_backend, world_size=args.world_size, rank=args.rank, \
            init_method=init_method, timeout=_exce_timeout
        )
