...
from transformer_sp.util import (
    get_sequence_parallel_group,
    get_sequence_parallel_size,
    enable_sequence_parallel,get_sequence_parallel_group_rank
)
    ...
    def rotary(self, t, **kwargs):
        if self.pnp:
            ...
            def reshape_freq(freqs):
                #t shape : B Nmu_head (t*w(h) C_head
                #          torch.Size([1, 48, 9450, 64])
                #          9450 = 7*30*45 = (T-1)/4+1 * latent_height/2 * latent_weight/2
                #actual input shape
                # frame = t.shape[2]
                frame = kwargs['latent_t']
                height = kwargs['latent_height']//2
                width = kwargs['latent_width']//2
                freqs = rearrange(freqs, '(t h w) d -> t h w d', h=self.height, w=self.width, t=self.compressed_num_frames)
                freqs = freqs[:frame, :height, :width, :]
                freqs = rearrange(freqs, 't h w d -> (t h w) d', h=height, w=width, t=frame).contiguous()
...
class FinalLayerMixin(BaseMixin):
    def __init__(
        ...
        # sp of transformer
        self.t_sp_size = get_sequence_parallel_size()
    ...
    def final_forward(self, logits, **kwargs):
        ...
        pad = 0
        kwargs["pad"] = pad
        kwargs["text_length_split"] = text_length

        x, emb = logits[:, kwargs["text_length_split"] :, :], kwargs["emb"]  # x:(b,(t n),d)
        ...
        return unpatchify(
            ...
            c=self.out_channels,
            p=self.patch_size,
            w=kwargs['latent_width']//self.patch_size,
            h=kwargs['latent_height']//self.patch_size,
...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        **kwargs,
        ...
        if enable_sequence_parallel():
            kwargs["text_length_split"] = text_length
            kwargs["sp_gp"] = get_sequence_parallel_group()
            # Enable splitting of sp only in the first layer.
            if layer_id == 0:
                # img_hidden_states.shape[1] = (max_num_frames // 4 + 1) * (img_h / 8 / 2) * (img_w / 8 / 2)
                assert img_hidden_states.shape[1] % self.t_sp_size == 0, \
                        f"Tweak max_num_frames for sp-divisibility. {img_hidden_states.shape}"
                img_hidden_states = split_sequence(img_hidden_states, get_sequence_parallel_group(), dim=1, grad_scale="down")
        ...
        # modality-separate-norm & self full attention (b, (t n), d)
        ...
        if enable_sequence_parallel():
            text_attention_output = attention_output[:, :kwargs["text_length_split"]]  # (b,n,d)
            img_attention_output = attention_output[:, kwargs["text_length_split"]:]  # (b,(t n),d)
        else:
        ...
        if enable_sequence_parallel():
            img_mlp_output = mlp_output[:, kwargs["text_length_split"]:]  # vision (b,(t n),d)
            text_mlp_output = mlp_output[:, :kwargs["text_length_split"]]  # language (b,n,d)
        else:
...
class DiffusionTransformer(BaseModel):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        global_src_rank = get_sequence_parallel_group_rank() * sp_size
        torch.distributed.broadcast(x, src=global_src_rank, group=get_sequence_parallel_group())
        b, t, d, h, w = x.shape
        ...
        kwargs['latent_width'] = w
        kwargs['latent_height'] = h
        kwargs['latent_t'] = t
