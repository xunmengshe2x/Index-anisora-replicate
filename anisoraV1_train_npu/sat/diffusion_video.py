...
class SATVideoDiffusionEngine(nn.Module):
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        if self.noised_image_input:
            ...
            if not self.training_from_vae_encoded_condition:
                ...
                if np.random.randint(2)>0:
                    ...
                    index_2 = None
                # image=self.scale_factor*x[:, :, 0:1]
                # image_last=self.scale_factor*x[:, :, -2:-1]
        ...
        if self.noised_image_input:
            ...
            if self.noised_image_all_concat:
                ...
                # image = torch.concat([torch.zeros_like(x[:, 1:]), image], dim=1)
                # image = torch.concat([image, torch.zeros_like(x[:, 1:-1]), image_last], dim=1)
                ...
                if index_2 is not None:
                    if index_2>index_:
                        ...
                        # print('index_', index_, 'index_2', index_2)
                        # print('fe_0', fe_0.shape)
                        # print('image', image.shape)
                        # print('fe_1_A', fe_1_A.shape)
                        # print('image_2', image_2.shape)
                        # print('fe_1_B', fe_1_B.shape)
                        ...
                        # print('index_2', index_2,'index_', index_)
                        # print('fe_0_A', fe_0_A.shape)
                        # print('image_2', image_2.shape)
                        # print('fe_0_B', fe_0_B.shape)
                        # print('image', image.shape)
                        # print('fe_1', fe_1.shape)
    ...
    # def mock_vae_encoded_data(self, frames):
    #     batch, frame, channels, height, weight = frames.shape
    #     encoded_frame_num = (frame - 1) // 4 + 1
    #     encoded_channels = self.vae_chan_out_dim
    #     encoded_height = height // 8
    #     encoded_weight = weight // 8
    #     new_shape = (batch, encoded_frame_num, encoded_channels, encoded_height, encoded_weight)
    #     fake_encoded_output = torch.rand(new_shape, dtype=frames.dtype, device=frames.device)
    #     return fake_encoded_output
