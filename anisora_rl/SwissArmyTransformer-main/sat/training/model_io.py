...
def save_ds_checkpoint(iteration, model, lr_scheduler, args, use_ema = False):
    ...
    if not use_ema:
        ...
        #save_ds_checkpoint_no_optim_wo_ref(model, args.save, str(iteration)+'-ema', client_state=sd)
...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None):
    ...
    for k in sd['module']:
        if k.startswith('model'):
            new_sd['module'][k[len("model")+1:]] = sd['module'][k]
        elif k.startswith('conditioner'):
            cond['module'][k[len("conditioner")+1:]] = sd['module'][k]
    sd = new_sd
    
    if hasattr(model, 'module'):
        module = model.module
    else: # inference without deepspeed
        module = model.model  # run there
        ref_module = model.ref_model
        cond_module = model.conditioner
    ...
    m_k,u_k = cond_module.load_state_dict(cond['module'], strict=False)
    ref_module.train = disabled_train
    ...
    if len(missing_keys) > 0:
        if args.mode == 'inference':
            ...
            model.reinit(mixin_names) # initialize mixins
