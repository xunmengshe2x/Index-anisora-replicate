...
def attention_forward_default(self, hidden_states, mask, **kw_args):
    ...
    if enable_sequence_parallel():
        q_text = query_layer[:, :, :kw_args["text_length_split"]]
        q = query_layer[:, :, kw_args["text_length_split"]:]
        k_text = key_layer[:, :, :kw_args["text_length_split"]]
        k = key_layer[:, :, kw_args["text_length_split"]:]
        v_text = value_layer[:, :, :kw_args["text_length_split"]]
        v = value_layer[:, :, kw_args["text_length_split"]:]

        q = all_to_all_with_pad(q,get_sequence_parallel_group(),scatter_dim=1,gather_dim=2)
        q_text = all_to_all_with_pad(q_text,get_sequence_parallel_group(),scatter_dim=1,gather_dim=2)
        k = all_to_all_with_pad(k,get_sequence_parallel_group(),scatter_dim=1,gather_dim=2)
        k_text = all_to_all_with_pad(k_text,get_sequence_parallel_group(),scatter_dim=1,gather_dim=2)
        v = all_to_all_with_pad(v,get_sequence_parallel_group(),scatter_dim=1,gather_dim=2)
        v_text = all_to_all_with_pad(v_text,get_sequence_parallel_group(),scatter_dim=1,gather_dim=2)
        if(kw_args["pad"]>0):
            q=q[:,:,kw_args["pad"]:]
            k=k[:,:,kw_args["pad"]:]
            v=v[:,:,kw_args["pad"]:]
            q_text=q_text[:,:,:-kw_args["pad"]]
            k_text=k_text[:,:,:-kw_args["pad"]]
            v_text=v_text[:,:,:-kw_args["pad"]]
        text_length=q_text.shape[2]

        query_layer=torch.cat([q_text,q],dim=2)
        key_layer=torch.cat([k_text,k],dim=2)
        value_layer=torch.cat([v_text,v],dim=2)

    context_layer = attention_fn(query_layer, key_layer, value_layer, mask, dropout_fn, **kw_args)
    if enable_sequence_parallel():
        pad=kw_args["pad"]
        encoder_hidden_states = context_layer[:, :, :text_length+pad]
        context_layer = context_layer[:, :, text_length-pad:]
        context_layer = all_to_all_with_pad(context_layer, get_sequence_parallel_group(), scatter_dim=2, gather_dim=1)
        encoder_hidden_states = all_to_all_with_pad(encoder_hidden_states, get_sequence_parallel_group(), scatter_dim=2, gather_dim=1)
        context_layer = torch.cat([encoder_hidden_states, context_layer], dim=2)  
