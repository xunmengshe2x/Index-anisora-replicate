...
class FinalLayerMixin(BaseMixin):
    ...
    def final_forward(self, logits, **kwargs):
        ...
        pad = 2 if self.cp_size == 4 else 0
        kwargs["pad"] = pad
        if enable_sequence_parallel():
            kwargs["text_length_split"] = (text_length+pad)//self.cp_size
        else:
...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        **kwargs,
        ...
        if(hidden_states.shape[1]>= (kwargs['latent_height']//2 * kwargs['latent_width']//2 * kwargs['latent_t'] + text_length)):
            text_hidden_states = hidden_states[:, :text_length+pad]  # (b,n,d)
            img_hidden_states = hidden_states[:, text_length-pad:]  # (b,(t n),d)

            if enable_sequence_parallel():
                text_hidden_states = split_sequence(text_hidden_states, get_sequence_parallel_group(), dim=1, grad_scale="down")
                img_hidden_states = split_sequence(img_hidden_states, get_sequence_parallel_group(), dim=1, grad_scale="down")
        else:
            text_hidden_states = hidden_states[:, :kwargs["text_length_split"]]  # (b,n,d)
            img_hidden_states = hidden_states[:, kwargs["text_length_split"]:]  # (b,(t n),d)
