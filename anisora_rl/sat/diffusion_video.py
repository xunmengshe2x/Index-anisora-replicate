...
class SATVideoDiffusionEngine(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        untrainable_prefixs = model_config.get('untrainable_prefixs', ['first_stage_model', 'conditioner', 'ref_model'])
        ...
        ref_model = instantiate_from_config(network_config)
        ...
        with torch.autocast("cuda", enabled=not self.disable_first_stage_autocast):
            for n in range(n_rounds):
                ...
                out = self.first_stage_model.decode(
                    z[n * n_samples: (n + 1) * n_samples], use_cp=use_cp, **kwargs
                )
                all_out.append(out)
    ...
    @torch.no_grad()
    def encode_first_stage_ref(self, x, batch):
        frame = x.shape[2]
        ...
        with torch.autocast("cuda", enabled=not self.disable_first_stage_autocast):
            for n in range(n_rounds):
                out = self.first_stage_model.encode(x[n * n_samples : (n + 1) * n_samples], use_cp=False if x.shape[2]==1 else True)
                all_out.append(out)
    ...
    def shared_step(self, batch: Dict) -> Any:
        x = self.get_input(batch) #x.shape torch.Size([1, 32, 11, 60, 90])  b,2*c,f,h,w
        ...
        if self.noised_image_input:
            image_w = x_w[:, :, 0:1]
            image_w = self.add_noise_to_first_frame(image_w)
            image_w = self.encode_first_stage(image_w, batch) #image.shape torch.Size([1, 16, 1, 60, 90])
            image_l = x_l[:, :, 0:1]
            image_l = self.add_noise_to_first_frame(image_l)
            image_l = self.encode_first_stage(image_l, batch) #image.shape torch.Size([1, 16, 1, 60, 90])
        image = torch.concat([image_w, image_l], dim=1)
        x = self.encode_first_stage(x, batch)
        x = x.permute(0, 2, 1, 3, 4).contiguous() # torch.Size([1, 11, 32, 60, 90])

        if self.noised_image_input:
            image = image.permute(0, 2, 1, 3, 4).contiguous() # torch.Size([1, 1, 32, 60, 90])
