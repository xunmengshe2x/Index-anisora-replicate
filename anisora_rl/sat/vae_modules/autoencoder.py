...
from vae_modules.cp_enc_dec import _conv_split_v2, _conv_split, _conv_gather
...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def encode(
        ...
        unregularized: bool = False,            **kwargs,
    ) -> Union[torch.Tensor, Tuple[torch.Tensor, dict]]:
        # print(101010111111111,x.shape)
...
class VideoAutoencoderInferenceWrapper(VideoAutoencodingEngine):
    ...
    def encode(
        ...
        input_cp: bool = False,
        output_cp: bool = False,            use_cp:bool = True,
    ) -> Union[torch.Tensor, Tuple[torch.Tensor, dict]]:
        if self.cp_size <= 1:
            use_cp = False
        if self.cp_size > 0 and use_cp and not input_cp:
            ...
            _cp_output_recv = []
            _cp_group_member = torch.distributed.get_process_group_ranks(get_context_parallel_group())
            for i in _cp_group_member:
                torch.distributed.broadcast(x, src=i, group=get_context_parallel_group())
                tmp = _conv_split(x, dim=2, kernel_size=1)
                _cp_output_recv.append(tmp)
            # print_all(f"################################################################", log_file="tmp.txt")
            # print_all(f"len(recv)-{len(_cp_output_recv)}", log_file="tmp.txt")
            # print_all(f"recv[0].shape-{_cp_output_recv[0].shape}", log_file="tmp.txt")
            # print_all(f"recv[-1].shape-{_cp_output_recv[-1].shape}", log_file="tmp.txt")
            # print_all(f"recv.dtype-{_cp_output_recv[0].dtype}", log_file="tmp.txt")

        # print(22222222222222,self.cp_size,use_cp,output_cp)
        if self.cp_size > 0 and use_cp and not output_cp:
            _cp_group_member = torch.distributed.get_process_group_ranks(get_context_parallel_group())
            _global_rank = torch.distributed.get_rank()
            _cp_group_rank = _global_rank % self.cp_size
            _z_list = []
            # Process data produced on rank-{_cp_group_member} with gather.
            for i in _cp_group_member:
                idx = i % self.cp_size
                x = _cp_output_recv[idx]
                ...
                '''
                    _cp_group_rank==0的是多的1个，其他是少的1个
                    23333333333 z.shape-torch.Size([1, 16, 10, 60, 90]) 1
                    23333333333 z.shape-torch.Size([1, 16, 11, 60, 90]) 0

                '''
                # print(23333333333,f"z.shape-{z.shape}",_cp_group_rank)#41=11=65
                if(_cp_group_rank==0):
                    big_shape=z.shape[2]
                    small_shape=big_shape-1
                else:
                    small_shape=z.shape[2]
                    big_shape=small_shape+1
                if 0 == len(_z_list):
                    _z_list = [torch.rand(1, 16, big_shape, z.shape[3], z.shape[4], dtype=z.dtype, device=z.device) for _ in range(self.cp_size)]

                if z.shape[2] == small_shape:
                    z = torch.cat([z, z[:, :, 0:1, :, :]], dim=2).contiguous()

                # print_all(f"z.shape-{z.shape}", log_file="tmp.txt")
                if idx == _cp_group_rank:
                    # print_all(f"G0, z.shape-{z.shape}", log_file="tmp.txt")
                    torch.distributed.gather(z, _z_list, i, group=get_context_parallel_group())
                else:
                    # print_all(f"G1, z.shape-{z.shape}", log_file="tmp.txt")
                    torch.distributed.gather(z, None, i, group=get_context_parallel_group())
                # print_all(f"G2", log_file="tmp.txt")
            for i in _cp_output_recv:
                del i

            for i in range(1, len(_z_list)):######去pad
                _z_list[i] = _z_list[i][:, :, :-1, :, :]
                # print(5555555555,_z_list[i].shape)

            z = torch.cat(_z_list, dim=2).contiguous()
            # print_all(f"z.shape-{z.shape}", log_file="tmp.txt")
            for i in _z_list:
                del i
        else:
    ...
    def encode_v1(
        ...
        if self.cp_size > 0 and not input_cp:
            ...
            _cp_output_send = _conv_split_v2(x, dim=2, kernel_size=1)
            _cp_output_recv = [torch.empty_like(_cp_output_send[0]) for _ in range(self.cp_size)]
            ...
            # print_all(f"################################################################", log_file="tmp.txt")
            # print_all(f"x.shape-{x.shape}", log_file="tmp.txt")
            # print_all(f"_cp_group_member-{_cp_group_member}", log_file="tmp.txt")
            # print_all(f"_cp_group_rank-{_cp_group_rank}", log_file="tmp.txt")
            # print_all(f"_global_rank-{_global_rank}", log_file="tmp.txt")
            # print_all(f"len(send)-{len(_cp_output_send)}", log_file="tmp.txt")
            # print_all(f"len(recv)-{len(_cp_output_recv)}", log_file="tmp.txt")
            # print_all(f"send.shape-{_cp_output_send[0].shape}", log_file="tmp.txt")
            # print_all(f"recv.shape-{_cp_output_recv[0].shape}", log_file="tmp.txt")
            # print_all(f"send.dtype-{_cp_output_send[0].dtype}", log_file="tmp.txt")
            # print_all(f"recv.dtype-{_cp_output_recv[0].dtype}", log_file="tmp.txt")
            # Process data produced on rank-{i} with scatter.
            for i in _cp_group_member:
                idx = i % self.cp_size
                if idx == _cp_group_rank:
                    torch.distributed.scatter(_cp_output_recv[idx], _cp_output_send, i, \
                                              group=get_context_parallel_group())
                    # print_all(f"torch.distributed.scatter done: src data on cp_rank-{idx}", log_file="tmp.txt")
                else:
                    torch.distributed.scatter(_cp_output_recv[idx], None, i, \
                                              group=get_context_parallel_group())

            for i in _cp_output_send:
                del i
        ...
        if self.cp_size > 0 and not output_cp:
            ...
            for i in _cp_group_member:
                ...
                if 0 == len(_z_list):
                    _z_list = [torch.empty_like(z) for _ in range(self.cp_size)]
...
class VideoAutoencoderInferenceWrapper_novae(VideoAutoencodingEngine):
    def __init__(
        self,
        cp_size=0,
        *args,
