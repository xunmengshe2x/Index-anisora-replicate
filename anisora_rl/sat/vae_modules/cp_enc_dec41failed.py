...
def _conv_split_v2(input_, dim, kernel_size, get_slice = False):
    cp_size = get_context_parallel_world_size()
    cp_need_split = range(cp_size)
    dim_limit = input_.size()[dim]
    stride = (dim_limit - kernel_size) // cp_size

    if get_slice == True:
        cp_group_idx = get_context_parallel_rank()
        start = cp_group_idx * stride
        end = start + stride
        output = input_.transpose(dim, 0)[start:end].transpose(dim, 0)
        return output.contiguous()
    else:
        output = []
        for i in cp_need_split:
            start = i * stride
            end = start + stride
            tmp = input_.transpose(dim, 0)[start:end].transpose(dim, 0)
            tmp = tmp.contiguous()
            output.append(tmp)
        return output

def _conv_split_v3(input_, dim, kernel_size):
    cp_group_member = torch.distributed.get_process_group_ranks(get_context_parallel_group())
    cp_groupe_size = get_context_parallel_world_size()
    stride = (input_.size()[dim] - kernel_size) // cp_groupe_size

    output = []
    for i in cp_group_member:
        cp_group_idx = i % cp_groupe_size
        if cp_group_idx == 0:
            tmp = input_.transpose(dim, 0)[: stride + kernel_size].transpose(dim, 0)
        else:
            tmp = input_.transpose(dim, 0)[cp_group_idx * stride + kernel_size \
                                           : (cp_group_idx + 1) * stride + kernel_size].transpose(dim, 0)

        output.append(tmp.contiguous())
    return output
...
def _conv_split(input_, dim, kernel_size):
    ...
    if cp_rank==0:input_=input_[:,:,:11]
    elif cp_rank==1:input_=input_[:,:,11:22]
    elif cp_rank==2:input_=input_[:,:,22:33]
    elif cp_rank==3:input_=input_[:,:,33:]
    input_=input_.contiguous()
    print(444444444444,cp_rank,input_.shape)

    ####49=13-12-12-12      13=4333
    ####41=12-12-12-5       11=3332
    ####41=12-11-10-8       11=3332
    ####41=11-11-11-8       11=3332
...
def _conv_gather_v2(input_, dim, kernel_size):
    cp_world_size = get_context_parallel_world_size()
    tensor_list = [torch.empty_like(input_) for _ in range(cp_world_size)]
    group = get_context_parallel_group()
    input_ = input_.contiguous()
    torch.distributed.all_gather(tensor_list, input_, group=group)
    output = torch.cat(tensor_list, dim=dim).contiguous()
...
def _conv_gather(input_, dim, kernel_size):
    ...
    # ###base:13-12-12-12:13就是源片，12的话拷贝第一帧
    ...
    print(22212312312313123,cp_rank,input_.shape)#11-11-11-8#torch.Size([1, 128, 11, 480, 720])
    # if(cp_rank==3):
    if(input_.shape[2]==8):
        input_=torch.cat([input_[:,:,:1]]*3+[input_],dim=2)#8补到11

    input_=input_.contiguous()
    print(12312312312313123,cp_rank,input_.shape)

    tensor_list =  [
        torch.empty_like(input_) for _ in range(cp_world_size)
    ]
...
class ContextParallelGroupNorm(torch.nn.GroupNorm):
    def forward(self, input_):
        ...
        # print_all(f"ContextParallelGroupNorm-input_.shape: {input_.shape}, is_gather: {gather_flag}", log_file="tmp.txt")
        if gather_flag:
            input_ = conv_gather_from_context_parallel_region(input_, dim=2, kernel_size=1)
        # print_all(f"ContextParallelGroupNorm-gather: {input_.shape}", log_file="tmp.txt")
        output = super().forward(input_)
        # print_all(f"ContextParallelGroupNorm-output.shape: {output.shape}", log_file="tmp.txt")
        if gather_flag:
            output = conv_scatter_to_context_parallel_region(output, dim=2, kernel_size=1)
        # print_all(f"ContextParallelGroupNorm-split: {output.shape}", log_file="tmp.txt")
        return output
...
class ContextParallelResnetBlock3D(nn.Module):
    ...
    def forward(self, x, temb, zq=None, clear_fake_cp_cache=True):
        ...
        # if isinstance(self.norm2, torch.nn.GroupNorm):
        #     h = conv_scatter_to_context_parallel_region(h, dim=2, kernel_size=1)
        # print_all(f"before h.shape: {h.shape}", log_file="tmp.txt")
        h = nonlinearity(h)
        h = self.dropout(h)
        h = self.conv2(h, clear_cache=clear_fake_cp_cache)
        # print_all(f"after h.shape: {h.shape}", log_file="tmp.txt")

        # print_all(f"in_channels: {self.in_channels}, out_channels: {self.out_channels}, use_conv_shortcut: {self.use_conv_shortcut}", log_file="tmp.txt")
        # print_all(f"before x.shape: {x.shape}", log_file="tmp.txt")
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                x = self.conv_shortcut(x, clear_cache=clear_fake_cp_cache)
            else:
                x = self.nin_shortcut(x)
        # print_all(f"after x.shape: {x.shape}", log_file="tmp.txt")
        return x + h
