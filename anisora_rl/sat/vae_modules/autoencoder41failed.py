...
from vae_modules.cp_enc_dec import _conv_split_v3, _conv_split_v2, _conv_split, _conv_gather
    ...
    def encode(
        ...
        if self.cp_size > 0 and not output_cp:
            ...
            for i in _cp_group_member:
                ...
                print(23333333333,_cp_group_rank,f"z.shape-{z.shape}")
                # print(23333333333,f"z.shape-{z.shape}")
                # print(23333333333,f"z.shape-{z.shape}")
                # print(23333333333,f"z.shape-{z.shape}")
                if 0 == len(_z_list):#3222=9=33#3332=11=41
                    # _z_list = [torch.rand(1, 16, 4, 60, 90, dtype=z.dtype, device=z.device) for _ in range(self.cp_size)]
                    _z_list = [torch.rand(1, 16, 3, 60, 90, dtype=z.dtype, device=z.device) for _ in range(self.cp_size)]
            ...
            # for i in range(1, len(_z_list)):
            #     _z_list[i] = _z_list[i][:, :, :-1, :, :]

            #####41的话只有最后一个要padding
            _z_list[-1] = _z_list[-1][:, :, :-1, :, :]
