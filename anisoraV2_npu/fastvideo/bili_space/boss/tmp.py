...
class BossClient:
    ...
    def get_client(cls):
        ...
        cfg["endpoint"] = "http://jssz-inner-boss.bilibili.co"
        cfg["access_key"] = "BysUD7QWq7K8A0rw"
        cfg["secret_key"] = "CjwDLp7UvejZVmgEeqMQM4kGzgsfD7Db"
        cfg["bucket"] = "danbooru2024"
...
if "__main__" == __name__:
    ...
    if handle:
        ...
        # tmp = handle.list_objects(basename=False, object_path="weight")
        tmp = handle.list_objects(basename=False, object_path="wan21/Wan2.1-I2V-14B-480P")
        # handle.download_file("pretrained/zhipu1224/pretrain-v2-512_i2vv2/latest")
        for k, v in enumerate(tmp):
            if k < 75:
                continue
            handle.download_file(v)
            print(f"Finished: {k}")
