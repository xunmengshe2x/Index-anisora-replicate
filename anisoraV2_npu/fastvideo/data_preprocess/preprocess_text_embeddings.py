...
def main(args):
        ...
        with torch.inference_mode():
            with torch.autocast("cuda", dtype=autocast_type):
                ...
                for idx, video_name in enumerate(data["filename"]):
                    ...
                    if boss_handle:
                        relative_path = os.path.relpath(prompt_embed_path, args.output_dir)
                        boss_handle.upload_file(os.path.abspath(prompt_embed_path), relative_path)
                        relative_path = os.path.relpath(prompt_attention_mask_path, args.output_dir)
                        boss_handle.upload_file(os.path.abspath(prompt_attention_mask_path), relative_path)
    ...
    if local_rank == 0:
        ...
        if boss_handle:
            src_file = os.path.abspath(os.path.join(args.output_dir, "videos2caption.json"))
