...
from ..bili_space.boss import BossClient

def save_checkpoint(model, optimizer, rank, output_dir, step, discriminator=False, save_optimizer=True):
    ...
    if rank <= 0 and not discriminator:
        ...
        #     # workaround.
        #     config_dict["dtype"] = str(config_dict.get("dtype", None))
    ...
    if rank == 0:
        ...
        if handle:
            src_dir = os.path.dirname(os.path.abspath(weight_path))
            handle.upload_folder(src_dir)
