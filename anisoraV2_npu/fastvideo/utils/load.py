...
# from fastvideo.bili_space.cog15B.flux import DiffusionTransformer
from sat.model.transformer import BaseTransformerLayer, SelfAttention, MLP
from sat.mpu.layers import ColumnParallelLinear, RowParallelLinear

from fastvideo.bili_space.wan.modules.model import WanModel, WanAttentionBlock
...
def load_transformer(
    ...
    #     assert dit_model_name_or_path is not None
    #     assert dit_model_cfg is not None
    #
    #     import argparse
    #     from functools import partial
    #     from sat.training.model_io import load_checkpoint
    #     from fastvideo.bili_space.cog15B.arguments import add_model_config_to_args, add_yaml_args_to_args
    #     from fastvideo.bili_space.cog15B.sgm.util import (
    #         instantiate_from_config,
    #     )
    #     inner_args = argparse.Namespace()
    #     inner_args = add_yaml_args_to_args(inner_args, cfg_file=dit_model_cfg)
    #     inner_args = add_model_config_to_args(inner_args, cfg_file=dit_model_cfg)
    #     transformer = instantiate_from_config(inner_args.model_config.network_config)
    #     if False:
    #         load_checkpoint(transformer, args=inner_args, load_path=dit_model_name_or_path, prefix="model.diffusion_model.")
    #
    #     loss_fn = (instantiate_from_config(inner_args.model_config.loss_fn_config) \
    #                if inner_args.model_config.loss_fn_config is not None else None)
    #     denoiser = (instantiate_from_config(inner_args.model_config.denoiser_config) \
    #                 if inner_args.model_config.denoiser_config is not None else None)
    #     # conditioner = (instantiate_from_config(inner_args.model_config.conditioner_config) \
    #     #                if inner_args.model_config.conditioner_config is not None else None)
    #     # conditioner.to("npu")
    #     fn = partial(loss_fn, denoiser=denoiser, conditioner=None)
    #     extra_rtv = {"loss_fn": fn, "scale_factor": inner_args.model_config.get("scale_factor", None)}
    elif model_type == "wan":
        assert pretrained_model_name_or_path is not None
        transformer = WanModel.from_pretrained(pretrained_model_name_or_path)
    else:
        raise ValueError(f"Unsupported model type: {model_type}")
    return transformer, extra_rtv
...
def get_no_split_modules(transformer):
    ...
    # elif isinstance(transformer, DiffusionTransformer):
    #     return (BaseTransformerLayer, MLP, SelfAttention, ColumnParallelLinear, RowParallelLinear, nn.Linear)
