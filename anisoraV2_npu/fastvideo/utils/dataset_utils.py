...
def get_length_grouped_indices(
    ...
    # split aligned megabatch into batches
    megabatches = [
...
class LengthGroupedSampler(Sampler):
    ...
    def __iter__(self):
        ...
        def distributed_sampler(lst, rank, batch_size, world_size, sp_size):
            result = []
            index = rank * batch_size // sp_size
            while index < len(lst):
                result.extend(lst[index : index + batch_size])
                index += batch_size * world_size // sp_size
            return result

        indices = distributed_sampler(indices, self.rank, self.batch_size, self.world_size, self.sp_size)
        return iter(indices)
