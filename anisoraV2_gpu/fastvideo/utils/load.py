...
class CogTextEncoderWrapper(nn.Module):
    def __init__(self, pretrained_model_name_or_path, device):
        super().__init__()
        self.text_encoder = T5EncoderModel.from_pretrained(
            os.path.join(pretrained_model_name_or_path, "text_encoder")
...
# from fastvideo.cogvideox.cogvideox_transformer_3d import CogVideoXTransformer3DModel,CogVideoXBlock
# from fastvideo.cogvideox.autoencoder_magvit import AutoencoderKLCogVideoX
# import sys
# sys.path.append("/DATA/bvac/personal/wan21/Wan2.1-main")
...
def load_transformer(
    ...
    #         dit_model_name_or_path, subfolder="transformer", torch_dtype=master_weight_type
    #     )
    elif model_type == "wan":
...
def load_vae(model_type, pretrained_model_name_or_path):
    ...
    #         # pretrained_model_name_or_path, subfolder="vae", revision=args.revision, variant=args.variant
    #         pretrained_model_name_or_path, subfolder="vae", revision=None, variant=None,torch_dtype=torch.bfloat16
    #     ).to("cuda")
    #     vae.eval()
    #     fps=233333
    #     autocast_type = torch.bfloat16
...
def load_text_encoder(model_type, pretrained_model_name_or_path, device):
    ...
    # elif model_type=="cog":
    #     text_encoder = CogTextEncoderWrapper(pretrained_model_name_or_path, device)
    else:
        raise ValueError(f"Unsupported model type: {model_type}")
    return text_encoder
...
def get_no_split_modules(transformer):
    ...
    # elif isinstance(transformer, CogVideoXTransformer3DModel):
