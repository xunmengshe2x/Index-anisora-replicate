...
class RFLOW:
    ...
    def sample(
        ...
        for i, t in progress_wrap(list(enumerate(timesteps))):
            ...
            # torch.save(pred_uncond.detach(),'feats/'+str(i)+'_'+str(int(t[0]))+'_uncond.pt')
            # torch.save(z.detach(),'feats/'+str(i)+'_'+str(int(t[0]))+'_z.pt')
            ...
            ######SAVE
            # torch.save(dt.detach(),'feats/'+str(i)+'_'+str(int(t[0]))+'_dt.pt')
            ...
            ######SAVE
            # torch.save(mask_t_upper.detach(),'feats/'+str(i)+'_'+str(int(t[0]))+'_mask_t.pt')
            # torch.save(x0.detach(),'feats/'+str(i)+'_'+str(int(t[0]))+'_x0.pt')
            # torch.save(z.detach(),'feats/'+str(i)+'_'+str(int(t[0]))+'_zfinal.pt')
