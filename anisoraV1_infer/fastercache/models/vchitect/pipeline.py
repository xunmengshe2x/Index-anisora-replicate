...
class VchitectXLPipeline():
    ...
    def _get_clip_prompt_embeds(
        ...
        text_encoder = clip_text_encoders[clip_model_index].cuda()
    ...
    def acc_call(
        ...
        for i, t in tqdm(enumerate(timesteps)):
            ...
            if self.counter > 30 and self.counter %5!=0:
                # expand the latents if we are doing classifier free guidance
                ...
                noise_pred_text = self.transformer.my_forward(
                    hidden_states=latent_model_input[1,:].unsqueeze(0),
                    timestep=timestep,
                    encoder_hidden_states=prompt_embeds[1,:].unsqueeze(0),
                    pooled_projections=pooled_prompt_embeds[1,:].unsqueeze(0),
                ...
                lf_c, hf_c = fft(noise_pred_text.float())
                
                if self.counter < 80:
                    self.delta_lf = self.delta_lf * 1.05
                if self.counter > 60:
                ...
                # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
                timestep = t.expand(latents.shape[0])
                noise_pred_uncond = self.transformer.my_forward(
                    hidden_states=latent_model_input[0,:].unsqueeze(0),
                    timestep=timestep,
                    encoder_hidden_states=prompt_embeds[0,:].unsqueeze(0),
                    pooled_projections=pooled_prompt_embeds[0,:].unsqueeze(0),
                ...
                # torch.Size([40, 16, 54, 96]) torch.Size([40, 16, 54, 96])
                # exit(0)
                lf_c, hf_c = fft(noise_pred_text.float())
                lf_uc, hf_uc = fft(noise_pred_uncond.float())

                self.delta_lf = lf_uc - lf_c
                self.delta_hf = hf_uc - hf_c
                # self.delta_cfg = noise_pred_uncond - noise_pred_text
    ...
    def mydelta_call(
        ...
        for i, t in tqdm(enumerate(timesteps)):
            ...
            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            timestep = t.expand(latents.shape[0])
            noise_pred_uncond = self.transformer.my_delta_forward(
                hidden_states=latent_model_input[0,:].unsqueeze(0),
                timestep=timestep,
                encoder_hidden_states=prompt_embeds[0,:].unsqueeze(0),
                pooled_projections=pooled_prompt_embeds[0,:].unsqueeze(0),
            ...
            noise_pred_text = self.transformer.my_delta_forward(
                hidden_states=latent_model_input[1,:].unsqueeze(0),
                timestep=timestep,
                encoder_hidden_states=prompt_embeds[1,:].unsqueeze(0),
                pooled_projections=pooled_prompt_embeds[1,:].unsqueeze(0),
    ...
    def mydeltaV1_call(
        ...
        for i, t in tqdm(enumerate(timesteps)):
            ...
            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            timestep = t.expand(latents.shape[0])
            noise_pred_uncond = self.transformer.my_deltaV1_forward(
                hidden_states=latent_model_input[0,:].unsqueeze(0),
                timestep=timestep,
                encoder_hidden_states=prompt_embeds[0,:].unsqueeze(0),
                pooled_projections=pooled_prompt_embeds[0,:].unsqueeze(0),
            ...
            noise_pred_text = self.transformer.my_deltaV1_forward(
                hidden_states=latent_model_input[1,:].unsqueeze(0),
                timestep=timestep,
                encoder_hidden_states=prompt_embeds[1,:].unsqueeze(0),
                pooled_projections=pooled_prompt_embeds[1,:].unsqueeze(0),
