...
class CogVideoXPipeline(VideoSysPipeline):
    ...
    def __init__(
        ...
        if config.model_path == "THUDM/CogVideoX-2b":
            dtype = torch.float16
        self._dtype = dtype
        cache_dir="/DATA/bvac/personal/opensora/zhipu/pretrained/hf2b/cache"
        if transformer is None:
            transformer = CogVideoXTransformer3DModel.from_pretrained(
                config.model_path, subfolder="transformer", torch_dtype=self._dtype,cache_dir=cache_dir
            )
        if vae is None:
            vae = AutoencoderKLCogVideoX.from_pretrained(config.model_path, subfolder="vae", torch_dtype=self._dtype,cache_dir=cache_dir)
        if tokenizer is None:
            tokenizer = T5Tokenizer.from_pretrained(config.model_path, subfolder="tokenizer",cache_dir=cache_dir)
        if text_encoder is None:
            text_encoder = T5EncoderModel.from_pretrained(
                config.model_path, subfolder="text_encoder", torch_dtype=self._dtype,cache_dir=cache_dir
            )
    ...
    def generate(
        ...
        use_dynamic_cfg: bool = True,##default is False
        num_videos_per_prompt: int = 1,
        eta: float = 0.0,
