...
class CogVideoXImageToVideoPipeline(VideoSysPipeline):
    ...
    def prepare_latents_2img(
        ...
        self.vae = self.vae.to(image.device)
        imgs=torch.cat([image,image_last],0)
        ...
        image_latents,image_latents_last=img_latents
    ...
    def generate(
        ...
        if len(img_list)==1:
            img = self.video_processor.preprocess(img_list[0], height=height, width=width,resize_mode="crop").to(
            device, dtype=prompt_embeds.dtype
        )
            latents, image_latents = self.prepare_latents(
                img,
        ...
        if len(img_list)==2:
            ...
            img_last = self.video_processor.preprocess(img_list[1], height=height, width=width,resize_mode="crop").to(
            device, dtype=prompt_embeds.dtype
        )
        ...
        if len(img_list)==3:
            ...
            img_mid = self.video_processor.preprocess(img_list[1], height=height, width=width,resize_mode="crop").to(
            device, dtype=prompt_embeds.dtype
        )
            img_last = self.video_processor.preprocess(img_list[2], height=height, width=width,resize_mode="crop").to(
            device, dtype=prompt_embeds.dtype
        )
