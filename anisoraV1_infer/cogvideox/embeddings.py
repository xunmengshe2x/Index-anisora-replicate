...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        #         )
            ...
            # #print('num_frames:', num_frames, 'compression:', self.temporal_compression_ratio,)
            # if (
...
def get_3d_rotary_pos_embed(
    embed_dim, crops_coords, grid_size, temporal_size, pos_emb_len, theta: int = 10000, use_real: bool = True
    ...
    #print('grid_t:', grid_t.shape, 'freqs_t:', freqs_t)
    # BroadCast and concatenate temporal and spaial frequencie (height and width) into a 3d tensor
    def combine_time_height_width(freqs_t, freqs_h, freqs_w, pos_emb_len):
        freqs_t = freqs_t[:, None, None, :].expand(
            -1, grid_size_h, grid_size_w, -1
        )  # temporal_size, grid_size_h, grid_size_w, dim_t
        #print('freqs_t concat:', freqs_t.shape)
        
        freqs_h = freqs_h[None, :, None, :].expand(
            temporal_size, -1, grid_size_w, -1
        )  # temporal_size, grid_size_h, grid_size_2, dim_h
        #print('freqs_h concat:', freqs_h.shape)

        freqs_w = freqs_w[None, None, :, :].expand(
            temporal_size, grid_size_h, -1, -1
        )  # temporal_size, grid_size_h, grid_size_2, dim_w
        
        #print('freqs_w concat:', freqs_w.shape)
        freqs = torch.cat(
            [freqs_t, freqs_h, freqs_w], dim=-1
        )  # temporal_size, grid_size_h, grid_size_w, (dim_t + dim_h + dim_w)
        #pdb.set_trace()
        freqs = freqs[:pos_emb_len, :, :, :]
        #print('new freqs:', freqs.shape)
        
        freqs = freqs.view(
            pos_emb_len * grid_size_h * grid_size_w, -1
        )  # (temporal_size * grid_size_h * grid_size_w), (dim_t + dim_h + dim_w)
        return freqs
    ...
    cos = combine_time_height_width(t_cos, h_cos, w_cos, pos_emb_len)
    sin = combine_time_height_width(t_sin, h_sin, w_sin, pos_emb_len)
    return cos, sin
