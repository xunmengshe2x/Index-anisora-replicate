...
class VideoDiffusionLoss(StandardDiffusionLoss):
    ...
    def __call__(self, network, denoiser, conditioner, input, batch):
        ...
        global_src_rank = get_sequence_parallel_group_rank() * get_sequence_parallel_size()
        torch.distributed.broadcast(idx, src=global_src_rank, group=sp_group)
        torch.distributed.broadcast(noise, src=global_src_rank, group=sp_group)
        torch.distributed.broadcast(alphas_cumprod_sqrt, src=global_src_rank, group=sp_group)
