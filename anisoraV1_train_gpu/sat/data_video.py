...
VIDEO_size = {"480p": [400, 720], "640p": [640, 960], "720p": [720 , 1280], "1080p": [1088, 1632]}
...
class SFTDataset(Dataset):
    def __load_video_info__(self, jsonfile, video_base_dir, fps, bucket_config, skip_frms_num):
        ...
        for size, F_dict in bucket_config.items():
            ...
            for num_frames, p in F_dict.items():
                if p>0:
                    ...
                    self.videos_list_real[str([video_size, num_frames])] = []
        ...
        def find_nearest_smaller_nframe(n, fps_oir):
            ...
            # if max_item is None:
            #     max_item = [[480, 720], 25]
            # if(len(ok_items)>0):max_item=random.choice(ok_items)
            return max_item
        ...
        for videoinfo in json_data:
            try:
                ...
                if (videoinfo['total_frames'] / videoinfo['fps']) < ( (self.min_num_frames)/fps ) :
                    continue
                video_path = videoinfo['vae_boss_path']#A
                video_path_real = videoinfo['hdfs_path']
                ...
                if cho_item[1]>0 and str(cho_item) in self.p.keys():
                    ...
                    self.videos_list_real[str(cho_item)].append(video_path_real)
        ...
        for i in range(len(del_key)):
            ...
            del self.videos_list_real[key_]
        ...
        for item in enumerate(self.p):
            key_ = item[1]
            self.p[key_] = float(self.p[key_]) if total_video_num==0 else float(self.p[key_])/total_video_num
    ...
    def __get_video__(self, video_path, video_path_real, video_size, num_frames, fps, skip_frms_num):

        #form hdfs video
        vr = VideoReader(uri=video_path_real, height=-1, width=-1)
        ...
        if end >= ori_vlen:
            print('video_path', video_path, 'start:', start, 'end:', end, 'fps:', fps, 'actual_fps:', actual_fps, 'num_frames:', num_frames, 'ori_vlen:', ori_vlen)
        temp_frms = vr.get_batch(np.arange(start, end))
        assert temp_frms is not None
        tensor_frms = torch.from_numpy(temp_frms) if type(temp_frms) is not torch.Tensor else temp_frms
        tensor_frms = tensor_frms[torch.tensor((indices - start).tolist())]
        ...
        return [tensor_frms, torch.load(video_path,map_location="cpu")[0]]
    ...
    def __init__(self, data_dir, video_base_dir,  bucket_config, fps, skip_frms_num=3):
        ...
        self.using_mock_data = os.environ.get("DATASET_USING_MOCK_DATA", "false") == "true"
        if self.using_mock_data:
            z_channels = int(os.environ.get("MOCK_VAE_ENCODERED_Z_CHANNELS", None))
            double_z = os.environ.get("MOCK_VAE_ENCODERED_DOUBLE_Z", 'False') == "True"
            self.vae_chan_out_dim = z_channels if double_z else z_channels // 2
            # print(f"mazhuoru, get from env, z channels {z_channels}, double_z {double_z}")
        self.cache=None

    def find_interval(self, a):
        for i in range(len(self.sample_p_new)):
            if a <= self.sample_p_new[i]:
                # print('a', a, 'self.sample_p_new[i]', self.sample_p_new[i])
                return i
        return f"{a} 不属于任何区间"
    ...
    def __getitem__(self, index):
        try:
            ...
            video_, vae_feature_ = self.__get_video__(self.videos_list[key_][index_], self.videos_list_real[key_][index_], self.video_size_list[key_][index_], self.num_frames_list[key_][index_], self.fps_list[key_][index_], self.skip_frms_num)
            item = {
                "mp4": vae_feature_,
                "mp4_real": video_,
                ...
                "vae_name": self.videos_list[key_][index_],
                "video_name": self.videos_list_real[key_][index_],
            }
            self.cache=item
        except:
            traceback.print_exc()
    ...
    def updatestep(self):
        ...
        # print('after updatestep: self.train_step', self.train_step)
...
if __name__ == "__main__":
    ...
    bucket_config={
        # ---480 720
        "480p":  {9: 0, 17: 0, 25: 0, 33: 0, 41: 0, 49: 0, 57: 0, 65: 0, 73: 0}, # 73only-ok
        # ---640 960
        "640p":  {9: 0, 17: 0, 25: 0, 33: 0, 41: 0, 49: 0, 57: 0, 65: 0}, # 49gg
        # ---720 1088
        "720p":  {9: 0, 17: 0, 25: 0, 33: 1, 41: 1, 49: 1, 57: 1, 65: 0}, # 49gg
        # ---1080 1632
        "1080p": {9: 0, 17: 0, 25: 0, 33: 0, 41: 0, 49: 0, 57: 0, 65: 0},
    }
    dataset = SFTDataset('/tmp/alluxio/llm_dataset/fuse/video_clips/bili_ogv_train_hq_0717.json',
               '/mnt/hdfs/video_clips/bili_ogv_clip/',
                bucket_config,
                16,
                3)
    for i in range(1):
        print(i,dataset[i]["mp4"][0].shape, dataset[i]["mp4"][1].shape)
        # dataset.__updatestep__()
    # item = dataset[2]
    # print(item)
    # print(item['mp4'].shape)