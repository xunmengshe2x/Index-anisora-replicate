...
def Normalize3D(in_channels, zq_ch, add_conv):
    return SpatialNorm3D(in_channels, zq_ch, norm_layer=nn.GroupNorm, freeze_norm_layer=False, add_conv=add_conv,
                         num_groups=32, eps=1e-6, affine=True)


class ResnetBlock3D(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
                 dropout, temb_channels=512, zq_ch=None, add_conv=False, pad_mode='constant'):
        super().__init__()
...
class MOVQDecoder3D(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels, resolution, z_channels,
                 give_pre_end=False, zq_ch=None, add_conv=False, pad_mode='first', temporal_compress_times=4,
                 **ignorekwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        ...
        self.mid.block_1 = ResnetBlock3D(in_channels=block_in,
                                         ...
                                         dropout=dropout,
                                         zq_ch=zq_ch,
                                         add_conv=add_conv, pad_mode=pad_mode)
...
class NewDecoder3D(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels, resolution, z_channels,
                 give_pre_end=False, zq_ch=None, add_conv=False, pad_mode='first', temporal_compress_times=4,
                 post_quant_conv=False, **ignorekwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        self.num_resolutions = len(ch_mult)
